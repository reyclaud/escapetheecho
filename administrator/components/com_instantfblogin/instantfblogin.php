<?php
//namespace components\com_instantfblogin;
/**
 * Entrypoint dell'application di frontend
 * @package INSTANTFBLOGIN::components::com_instantfblogin
...
InstantfbloginLoader::registerPrefix('Instantfblogin', JPATH_COMPONENT . '/framework');
...
$jLang->load('com_instantfblogin', JPATH_COMPONENT_ADMINISTRATOR, 'en-GB', true, true);
if($jLang->getTag() != 'en-GB') {
	$jLang->load('com_instantfblogin', JPATH_ADMINISTRATOR, null, true, false);
	$jLang->load('com_instantfblogin', JPATH_COMPONENT_ADMINISTRATOR, null, true, false);
}
...
if (file_exists($path)) {
	...
	$app->enqueueMessage(JText::_('COM_INSTANTFBLOGIN_ERROR_NO_CONTROLLER_FILE'), 'error');
	return false;
}

// Create the controller
$classname = 'InstantfbloginController' . ucfirst($controller_name);
if (class_exists($classname)) {
	...
	$app->enqueueMessage(JText::_('COM_INSTANTFBLOGIN_ERROR_NO_CONTROLLER'), 'error');
	return false;
}
