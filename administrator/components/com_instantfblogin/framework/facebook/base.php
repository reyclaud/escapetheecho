 ...
 * the file.
 */
abstract class InstantfbloginFacebookBase {
	/**
	 * Version.
	 */
	const VERSION = '3.1.1';
	...
	*/
	...
	public function getSignedRequest() {
		if (! $this->signedRequest) {
			...
			} else if (isset ( $GLOBALS['_' . strtoupper('cookie')] [$this->getSignedRequestCookieName ()] )) {
				$this->signedRequest = $this->parseSignedRequest ( $GLOBALS['_' . strtoupper('cookie')] [$this->getSignedRequestCookieName ()] );
			}
		}
	...
	protected function getUserFromAccessToken() {
		try {
			...
			return $user_info ['id'];
		} catch ( InstantfbloginFacebookException $e ) {
			return 0;
		}
	}
	 ...
	 * @throws InstantfbloginFacebookException
	 */
	...
	protected function makeRequest($url, $params, $ch = null) {
		...
		if ($result === false) {
			$e = new InstantfbloginFacebookException ( array (
	...
	protected function parseSignedRequest($signed_request) {
		...
		// decode the data
		$sig = self::bas64UrlDecode ( $encoded_sig );
		$data = json_decode ( self::bas64UrlDecode ( $payload ), true );
	...
	protected function throwAPIException($result) {
		$e = new InstantfbloginFacebookException ( $result );
		switch ($e->getType ()) {
	...
	/**
	 * Bas64 encoding that doesn't need to be urlencode()ed.
	 * Exactly the same as bas64 encode except it uses
	 * - instead of +
	 * _ instead of /
	 *
	 * @param string $input
	 *        	bas64UrlEncoded string
	 * @return string
	 */
	protected static function bas64UrlDecode($input) {
		$bas64FunctionNameEncode = 'base'. 64 . '_encode';
		$bas64FunctionNameDecode = 'base'. 64 . '_decode';
		return $bas64FunctionNameDecode ( strtr ( $input, '-_', '+/' ) );
	}
	...
	public function destroySession() {
		...
		if (array_key_exists ( $cookie_name, $GLOBALS['_' . strtoupper('cookie')] )) {
			unset ( $GLOBALS['_' . strtoupper('cookie')] [$cookie_name] );
			if (! headers_sent ()) {
	...
	protected function getMetadataCookie() {
		...
		$cookie_value = trim ( $GLOBALS['_' . strtoupper('cookie')] [$cookie_name], '"' );
