<?php
// namespace administrator\components\com_instantfblogin\framework\model;
/**
 *
 ...
 * @since 1.6
 */
interface IInstantfbloginModel {
	/**
 ...
 * @since 1.6
 */
class InstantfbloginModel extends JModelLegacy implements IInstantfbloginModel {
	/**
		...
		if (isset ( $idEntity ) && $idEntity) {
			try {
				...
				if (! $table->publish( $idEntity, $state )) {
					throw new InstantfbloginException ( $table->getError (), 'notice' );
				}
			} catch ( InstantfbloginException $e ) {
				$this->setError ( $e );
				return false;
			} catch ( Exception $e ) {
				$InstantfbloginException = new InstantfbloginException ( $e->getMessage (), 'notice' );
				$this->setError ( $InstantfbloginException );
				return false;
			}
		}
			...
			try {
				foreach ( $ids as $id ) {
					if ($table->load ( ( int ) $id )) {
						$table->id = 0;
						$table->name = JText::_ ( 'COM_INSTANTFBLOGIN_COPYOF' ) . $table->name;
						$table->published = 0;
	...
	public function getComponentParams() {
		...
		if($this->app instanceof JSite) {
			$this->componentParams = $this->app->getParams($this->option);
		} else {
			$this->componentParams = JComponentHelper::getParams($this->option);
		}
