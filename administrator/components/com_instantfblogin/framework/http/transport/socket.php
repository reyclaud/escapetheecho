<?php
// namespace components\com_instantfblogin\libraries\http\transport;
/**
 ...
 * @since 1.6
 */
class InstantfbloginHttpTransportSocket implements InstantfbloginHttpTransport {
	/**
	 ...
	 *  
	 * @since   11.3
	 * @throws  InstantfbloginExceptionRuntime
	 */
	public function __construct() {
		if (!function_exists('fsockopen') || !is_callable('fsockopen')) {
			throw new InstantfbloginExceptionRuntime('Cannot use a socket transport when fsockopen() is not available.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  InstantfbloginExceptionRuntimes
	 */
	public function request($method, JUri $uri, $data = null, array $headers = null, $timeout = 120, $userAgent = null) {
		...
		if (is_resource($connection)) {
			...
			if ($meta['timed_out']) {
				throw new InstantfbloginExceptionRuntime('Server connection timed out.', 'error');
			}
		} else {
			throw new InstantfbloginExceptionRuntime('Not connected to server.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  InstantfbloginExceptionUnexpectedValue
	 */
	protected function getResponse($content) {
		// Create the response object.
		$return = new InstantfbloginHttpResponse;
		...
		if (is_numeric($code)) {
			...
			throw new InstantfbloginExceptionUnexpectedValue('No HTTP response code found.', 'notice');
		}
	...
	protected function connect(JUri $uri, $timeout = null) {
		...
		if (!$connection) {
			throw new InstantfbloginExceptionRuntime($err, 'error', $errno);
		}
