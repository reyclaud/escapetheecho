...
class PlgUserProfile extends JPlugin
{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		if ($app->isSite() || $name == 'com_users.user' || $name == 'com_admin.profile')
		{
			...
			$form->setValue('city','profile',$details->city);
			$form->setValue('region','profile',$details->region);
			$form->setValue('country','profile',$details->country);
			$form->setValue('postal_code','profile',$details->postal);
			//$lat = $details->loc;
			//echo $lat;
			//echo $myArray = explode(',', $lat);;
			...
			foreach($result as $key=>$value){
				...
				if($actual_link == 'http://escapetheecho.org/profile' || $actual_link == 'http://escapetheecho.org/profile/'){
					print_r("<div class = 'points' style='float:right;'>".$quiz.":". $result2 . " echoos coins </div>");
				}
			...
			$query2->select($db->quoteName(array('SubmissionValueId', 'FormId', 'SubmissionId', 'FieldName','FieldValue')));
			$query2->from($db->quoteName('jos_rsform_submission_values'));
			$query2->where($db->quoteName('FormId') ." = ".$result);
