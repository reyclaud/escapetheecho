...
jQuery(document).ready(function () {
    ...
    debatingGame = {
        ...
        ini: function () {
            ...
            if (jQuery(".flip-container").length > 4) {
                jQuery(".flip-container:nth-child(" + parseInt(flipContainer / 2) + ")").find(".back").css({left: 'auto', right: '-2px'});
                jQuery(".flip-container:gt(" + (parseInt(flipContainer / 2) - 1) + ")").find(".back").css({left: 0, right: 'auto', top: 'auto', bottom: 0, marginBottom: '0'});
        ...
        match: function () {
            if (debatingGame.countFlipped() > 1) {
                ...
                if ((jQuery("div.flipped.front:eq(0)").parent().attr("data-bind") == jQuery("div.flipped.front:eq(1)").parent().attr("data-bind") + '1') || (jQuery("div.flipped.front:eq(1)").parent().attr("data-bind") == jQuery("div.flipped.front:eq(0)").parent().attr("data-bind") + '1')) {
                    ...
                    jQuery("div.flipped").parent().parent().fadeOut(300).fadeIn(300, function () {
                        ...
                        position = "-" + (parseInt(jQuery(ths).position().left) - 1) + "px -" + jQuery(ths).position().top + "px";
                        bgsize = (jQuery(".flip-container-wrap").outerWidth() - 4) + "px";

                        jQuery(ths).css({backgroundImage: 'url(./templates/protostar/images/echo13.jpg)', backgroundSize: bgsize + " auto", backgroundPosition: position, backgroundRepeat: "no-repeat"});
                        ...
                        if (jQuery(".paired_cards").length >= jQuery(".flip-container").length) {
                            ...
                            jQuery(".next-level-wrap.pair-button-wrap, .save-coin-wrap").slideDown("fast", function () {
            ...
            } else if (jQuery(".flip-container").not(".paired_cards").length < 2) {
                jQuery('#audio_paired')[0].play();
